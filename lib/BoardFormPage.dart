...
class _BoardFormPageState extends State<BoardFormPage> {
  File? _selectedImage;        // for Android/iOS
  Uint8List? _webImage;        // for Web
  XFile? _pickedFile;          // keep original picked file (works both)
  String? _webImageName;       // filename for web
  ...
  void initState() {
    ...
    _renewalAtController = TextEditingController(text: widget.board?.renewalAt ?? "");
    _nextRenewalAtController = TextEditingController(text: widget.board?.nextRenewalAt ?? "");
    _renewalByController = TextEditingController(text: widget.board?.renewalBy ?? "");
    _createdByController = TextEditingController(text: widget.board?.createdBy ?? "");
  }
  ...
  Future<void> _saveBoard() async {
    ...
    try {
      ...
      final board = Board(
        id: widget.board?.id ?? 0, // ensure id is sent for update
      ...
      if (isEdit) {
        if (kIsWeb) {
          await ApiService.updateBoardWeb(board, _webImage, _webImageName);
        } else {
          await ApiService.updateBoard(board, imageFile: _selectedImage);
        }
      } else {
        if (kIsWeb && _webImage != null && _webImageName != null) {
          await ApiService.createBoardWeb(board, _webImage!, _webImageName!);
        } else {
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  else if (widget.board?.image.isNotEmpty == true)
